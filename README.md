...
- 词根词缀库：本项目自编（英语 `packages/data-pipeline/roots.json`、**俄语 `roots_ru.json`**，共 888 条词素，例词经词表校验；俄语部分由 `analyze_ru_morphemes.py` 从 Wiktionary 构词模板与词表复合切分**数据驱动**挖掘候选，人工审定含义与词源后入库，覆盖率用 `measure_ru_breakdown.mjs` 评测）。
