...
> 整理时间：`2026-09-16`（T55 后订正；**T82 事实性订正**）· 对应版本 **v0.10.0** · HEAD **`0dadb61`**（**T82 订正时点**；此前 `2a796c9` T55 结构快检裁决 / `05788be` / `224ce73` / `d558aa0`）· 15 个 tag
...
├── 🔧 工具 ─────── scripts/（**111** 个只读探针/实验台，**不是产品代码**）· _serve_static.mjs（UI 验证）
...
**三条最容易被新人踩错的**：
1. `scripts/` 里**绝大多数是历史实验脚本**（含 **5 个**结论已被推翻的，见 §3 D 档），**不是**要维护的产品工具 —— 新增工具请另立名字并登记到本文 §3。
...
| `scripts/` | **只读工具 + 历史留痕** | ✅ **111** 文件（时点 `0dadb61`） | 各角色自建、只读他人 | 见 §3 —— **产品代码不在这里**；**无任何 tracked 子目录文件 ⇒ 顶层 ≡ 递归** |
| `.board/` | **多 agent 台账** | ✅ **16** 文件（**顶层 9 个 `.md`** + `roles/` 3 + `structure/` **4**）（时点 `0dadb61`） | **按文件分角色** | 见 §4 |
| `docs/` | **人读权威文档** | ✅ **13** 文件（`docs/` 7 ＋ `docs/legacy/` 6） | 需求管理 agent | 见 §5 |
...
**已核验的入库计数**（**时点 `0dadb61`**，T82 实测）：`git ls-files` 共 **288** 个文件 = `scripts/` **111** + `packages/` 43 + `apps/` **94** + `.board/` **16** + `docs/` **13** + `data/` 1（`.gitkeep`）+ 根目录 **10**。（`apps/` 由 38 增至 94 = **v0.11.0 新增 `apps/mobile/`（Capacitor ＋ `android/` 原生工程）**；`scripts/` 由 99 回升至 111 = v0.11.0 期间新增探针。）
> ⚠ **本行数字的变迁链（留痕，勿删旧值）**：**253**（T55 实测 = `scripts/` 135 + …）→ **219**（时点 `17ef659`：T62 删 39 个零引用脚本，`scripts/` 135 → 99）→ **288**（时点 `0dadb61`：T82）。⇒ **同一批数字在同一天内第 5 次过时** —— 引用**必须带时点**（§11 计数纪律）。
...
## 3. `scripts/` — **111** 个文件的性质与保留策略（时点 `0dadb61`）
...
| 前缀 | 数量 | 作者 | 性质 | 保留策略 |
|---|---|---|---|---|
| `probe_*` | 21 | 主管（早期） | 只读探针，多用于 v0.8.0 的 D1 调查 | 保留（证据可追溯） |
| `probe_sup_*` | 8 | 主管（v0.9.0+） | 决策/审计探针 | 保留 |
| `probe_t*` | 46 | **测试 agent** | 独立验证探针（t15/t18/t20/t28/t29/t32/t36/t42/t46/t50/t64 轮次） | 保留 |
| `exp_*` | 21 | 开发 agent | 实验台（复刻引擎 + 机制判别） | 保留 |
| `out_*` | 8 | 各 agent | **输出快照**（`.txt`/`.json`） | 保留（报告引用它们） |
| `apply_*` | 2 | 主管 | 一次性落库脚本（v0.9.0 词素） | **不得重跑**（已落库，重跑会重复写入） |
| `check_*` / `build_*` | 2 / 1 | 主管 | 契约与诊断工具（`check_v10_ui_contract.mjs` · `check_sync_dto_contract.mjs` · `build_web_nospawn.mjs`） | 长期维护 |
| 无前缀 | 2 | 主管 | **普查工具** `audit_text_health.mjs` · `fix_readme_l144_corruption.mjs` | 长期维护 |
| **合计** | **111** | —— | **时点 `0dadb61`** —— T62 清理后为 **99**，v0.11.0 期间新增探针后回升至 **111** | —— |

**活跃工具（唯一需要维护的）**：
- `scripts/check_v10_ui_contract.mjs` — UI 文本级契约检查（**53 通过 / 0 失败**；`REQ.md` AC-16/AC-17 的机械判定入口。块 F 为条件式：`book_lang.mjs` 未加链时只 warn）
- `scripts/check_sync_dto_contract.mjs` — 同步服务 DTO 契约检查（**v0.11.0 期间新增**；⚠ 本 agent **未复跑其断言数**，引用时请自行实测）
...
**✅ `scripts/_tmp/` —— 根因已修，本项转终态（T82 实测）**：
- `packages/core/test/book_lang.mjs` 每次运行**仍在** `scripts/_tmp/booklang_tmp/run-<随机>/` 建临时合成库，但**收尾清理已成功**：T82 实测 `pnpm --filter @zidiankaifa/core test` ⇒ **exit 0**，其末行打印 **「临时库已清理：scripts\_tmp\booklang_tmp\run-Iix8zZ」**（**不再报 `EPERM, Permission denied`**）；跑前 / 跑后 `Get-ChildItem -Recurse -Force scripts/_tmp` 递归条目**均为 0**。
- **根因** = 泄漏连接使句柄未释放（`V11-OPENDB-LEAK`），已由开发 agent **T61 修复**（提交 **`2c908e8`**：`openDatabase()` 错误路径补 `db.close()`，**原错误原样抛出**）。
- ⇒ **T55/T60 的处置与 §11 熔断（「不得第三次重删」、「不删只记数」）已随根因解除**；门禁复跑**不再制造** `run-*` ⇒ **「临时残留 = 0」与「门禁跑绿」可以同时成立**（原 «二者只能保证其一» 的互斥约束**作废**）。
- ⚠ **另一种成因仍须辨认**：`packages/core/test/book_lang.mjs:1786-1789` 有「**仅断言全绿时才删临时目录、红态保留取证**」分支 ⇒ **将来若真再现 `run-*`，先判「红态留证（不是垃圾）vs 泄漏残渣」再处置**；纪律保留：**复跑后须与复跑次数对账**。
- 📌 **提请主管订正 `BASELINE.md` §7（本 agent 依 `BASELINE.md:118` 不直接改该文件）**：① `.board/` 文件数 **14 → 16**（时点 `0dadb61`）② 「每跑一次 `pnpm test` 新增 1 个 `run-*`」⇒ **实测恒 0**（根因已修）③ 「临时残留 = 0 与门禁跑绿互斥」**约束解除**。


---

## 4. `.board/` — 多 agent 文件中枢（**顶层 9 个 `.md`** · tracked **16** 文件 · 约 1.0 MB）（时点 `0dadb61`）

| 文件 | 行数 | **独占写者** | 内容 |
|---|---|---|---|
| `BASELINE.md` | 118 | **主管** | 冻结基线：HEAD/测试/词库指纹 + **三把尺子口径表 §4** + §7 结构预算（T55 订正） —— ⚠ **§7 第 3/4/5 条已过时**（`.board/` 14→**16** · `run-*` 实测恒 **0** · «互斥» 约束解除），**已提请主管订正**（本 agent 依 `BASELINE.md:118` 不直接改该文件）；**agent 数口径见 `.board/agents.md`**（T82：**总数 15**） |
...
| `STRUCTURE.md` | 503 | **结构管理 agent** | 结构台账：九项体检实测 · 清理记录 · §4.1–§4.4 档位与判据（含 `_tmp` 判据）· §17 裁决落地对照 · 越线提请 |
| `agents.md` | 120 | **结构管理 agent** | **agent 登记册**：受保护 4 · 业务常驻 3 · **结构席位 2**（出列 1 ＋ 在岗 1）· **agent 总数 15**（T82）· 临时 9 · 异常七类 · **§7 临时 agent 准入与结案纪律** · **§9 交接记录**（委任书 §14）。自检工具 = `structure/agents_audit.mjs` |
| `roles/*.md` | dev 71 / test 89 / **structure 261** | 各自 | 三份委任书；**structure-agent.md §14 = agent 结构管理** |
| `structure/*.mjs` | **4** 个只读工具 | **结构管理 agent** | `classify_mece.mjs`（§7 档位 MECE 校验）· `scan_dead_refs.mjs`（死引用扫描）· `recheck_zero_refs.mjs`（T62 零引用四通道复核）· `agents_audit.mjs`（**agent 登记册自检**：13 UUID / 保全清单 / 新增事实）；**须在仓库根运行** |
...
## 5. `docs/` — 人读权威文档（**13** 个文件 = `docs/` 7 ＋ `docs/legacy/` 6）
...
**九项体检**（每次快检逐项实测）：① 已跟踪文件总数 ② `scripts/` 文件数 ③ `scripts/` 顶层文件数 ④ `dist-release/` 递归体量 ⑤ `data/` 体量（**只报不删**）⑥ 缓存合计 ⑦ 临时残留 ⑧ 僵尸/废弃产物 ⑨ 单文件规模（字节 + 行数）⑩ **常驻合计恒为 5 = 受保护 4 ＋ 结构 1**（依裁决 B，`TASKS.md` §11.9；⚠ **若按「席位」计则为 6** —— 结构席位 2 = 出列 `2d4ba4d0` ＋ 在岗 `bb9e0148`，见 `.board/agents.md` §1.2）。
...
| 2026-09-16（**T82 事实性订正**） | 按实测订正 **5 处 ＋ 连带 7 处**（**仅事实，未改架构与归属**）：① `scripts/` **135 → 111** ＋ §3 前缀分档重测（`probe_*` 21 · `probe_sup_*` 8 · `probe_t*` 46 · `exp_*` 21 · `out_*` 8 · `apply_*` 2 · `check_*` 2 · `build_*` 1 · 无前缀 2 = **111**）② `.board/` **14 → 16**（`structure/` 2 → **4**）③ `docs/` **7 → 13** ④ §10 体检⑩「常驻 agent 数（恒为 4）」→ **「常驻合计恒为 5 = 受保护 4 ＋ 结构 1」** ⑤ HEAD `2a796c9` → **`0dadb61`**；连带：已跟踪总数 **219 → 288**（时点 `0dadb61`）· `apps/` 38 → **94**（v0.11.0 手机端）· `structure/*.mjs` **2 → 4** · `STRUCTURE.md` 488 → **503** · `agents.md` 250 → **120** · §0「含 3 个结论已被推翻」→ **5** · **§3 `_tmp` 块转终态**（根因 T61 `2c908e8` 已修；实测门禁 exit 0 且末行「临时库已清理」、`run-*` 恒 0、«互斥» 约束解除）。⚠ **主管 T82 清单所给 `scripts/` = 99 与 `.board/` = 15 均过时**（99 是 T62 时点值，v0.11.0 新增探针后回升至 111）⇒ **以实测为准**；并**提请主管订正 `BASELINE.md` §7 第 3/4/5 条**（依 `BASELINE.md:118`，本 agent 不直接改该文件） | 主管 T82 派单（三处错标订正）＋ 结构 agent 本单实测（`git ls-files` / `Get-ChildItem` / `pnpm --filter @zidiankaifa/core test`） |
