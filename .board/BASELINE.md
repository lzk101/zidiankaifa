...
| 维度 | 硬上限 | 每迭代净增配额 | 冻结时实测（v0.10.0 发布后，2026-09-16） | 余量 |
| --- | --- | --- | --- | --- |
| 已跟踪文件总数 | **300** | ≤ 25 | **251** | 49 |
| `scripts/` 文件数（递归） | **165** | ≤ 15 | **135** | 30 |
| `dist-release/` 体量（递归） | **1.5 GB** | 只留当前 + 上一版本 | 5.494 GB（121 文件）→ 清理执行中 | **−3.99 GB ⛔ 已越线**（已核准清理） |
| `data/` 体量（**只报不删**） | **4.5 GB** | 0 | 3.667 GB（53 文件） | 0.83 GB |
| 缓存合计（`.pnpm-store`/`.electron-cache`/`.eb-cache`） | **1.2 GB** | — | 0.791 GB | 0.41 GB |
| 临时残留（`_tmp/` `*.tmpdir/` `.tmp/`） | **0** | — | 0（T55 首轮已清；`booklang_tmp/run-*` 每跑门禁 +1，见下 ⚠） | — |
| 单文件字节 | **20 MB** | — | 最大已跟踪 = `scripts/out_v9_admission.json` **639,777 B**（3.0%） | — |
| `.board/*.md` 单文件行数 | **3000** | — | BOARD **2460** · EVIDENCE **2451** | ⚠ 余 540 / 549 |
...
> 3. **`.board/` 文件数**：`git ls-files .board` = **12**（含 `roles/` 下 4 份委任书），原 `PROJECT_STRUCTURE.md` 记 9 已过时。
