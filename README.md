...
- 🧩 **词根词缀拆解**：内置 **888 条词素库**（英语 456 + **俄语 432**：前缀 185 / 后缀 93 / 词根 154，其中 287 条由 Wiktionary 构词模板与词表复合切分**数据驱动**扩充），**贪心最长匹配**（一词多词素全命中、长词素优先、例词点击即查）；俄语拆解额外支持**单字符前缀紧跟验证**（в-/с-/у-/о-）、**屈折词尾吸收**（`-ость` → `-ости/-остью`）与 **ё/е 等价**（`весёлый` 命中 `весел-`），`телефон → теле- + фон-`、`водопровод → водо- + -провод`、`тоска → тоск-`
