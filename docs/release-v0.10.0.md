...
- **另有两条登记项**（非本版引入，测试 agent 的观察项，判定权在主管）：
  1. 「备份失败」路径会绕过 T45 后置校验。可达条件为磁盘不足 / 目录不可写 / I-O 错误；该路径**保留原表不迁移**（数据不损）。
...
2. **D2 通用空洞上限** —— 「词中空洞无上限」的通用机制（`землетрясение` 跳过 `ряс` 型）。
3. **假词根全库普查** —— 库内词根自身含语义错误（`каз-` / `дом-` / `дн-` 被当假词根不是算法问题，是**词素库的语义维护问题**）。
4. **`термостат` 取舍** —— 与 `-ной` 的交互（加了会吃掉真前缀 `термо-`）。
5. **`да-` 假族 26 词** —— 逐词核证有硬证据，但消除会误伤真族 10 词，需新机制。
