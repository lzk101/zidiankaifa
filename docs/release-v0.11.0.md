...
> ⚠ **本文档状态：草稿（DRAFT）**。撰写时点为 HEAD `09c74bb`。
...
| 产物 | `app-release.apk` = **3,277,739 B** |
...
| 项 | 状态 | 说明 |
|---|---|---|
| **账号与多用户隔离** | ❌ **未交付** | 服务端仍是**单个全局 token**、`book` 表**无用户维度**。⇒ 现状**只能服务「单人多设备」**；多人共用同一服务端会**互相看到并修改对方的生词本**。这是**发布阻断项**（`DEC-031`） |
| **离线查词** | ❌ **未交付** | 安装包内 `assets/` **仅 226.5 KB**（只有 web 壳），**不含任何词库文件**。⇒ **断网状态下无法查词** |
...
本文档撰写时，以下项**未通过**，故 v0.11.0 **不能发布**：

| # | 门禁 | 状态 |
|---|---|---|
| G1 | AC-27 用户隔离（`DEC-031` 发布阻断项） | ❌ 未落地 |
| G2 | 门禁 core 621/0 ＋ desktop 22/0 ＝ **643/0** | ⏳ 待 AC-27 落地后复测 |
| G4 | 包内 web 资源为**最新**构建 | ⏳ 当前与 `apps/web/dist` 一致，但二者同属 09-14 旧构建 |
| G5 | 四包版本号与 3 处文本统一为 `0.11.0` | ⏳ 现为 `0.10.0` |

⇒ **发布动作（bump / 打包 / 发 Release）在 G1–G6 全绿后才可执行。**
...
2. **服务端无用户隔离**：见 §二第 1 行 —— **不要把它暴露到公网**，也不要多人共用
3. **离线不可用**：见 §二第 2 行
4. **iOS 缺失**：见 §二第 3 行
