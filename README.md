...
- [ ] **词素库扩充 + 词素级语义核证**（v0.9.0 计划：补约 67–99 条经联网词源审定的俄语词根以提升拆解覆盖率（只改规则的零新增上限为 **36.54%**，不足以达 45%）；并消灭残余假词根——`сучить`→`уч-`、`однако`→`дн-`、`столп`→`стол-`、`казус`→`каз-` 等，根因是当前只做**字母级**判定（gap 字符 ∈ 前缀字母表）而不做**词素级**判定）
