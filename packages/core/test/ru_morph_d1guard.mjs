...
/* ================================================================== *
 * 冻结真值（v0.9.0 重标；各项均为实测值 + 口径说明，非转述）
 *
 * ★ v0.9.0 重标说明（主管 T29，主库落库 7 条词素后）：
 *   本次落库 суч- / суд- / домин- / -ной / столп- / казус- / однако- / пад- / тряс-，
 *   使「真前缀组」这一度量**换形**，各常量的实测值变动如下（全库端到端，口径见 AGENTS.md 铁律 7）：
 *
 *   常量              v0.8.0 实测   v0.9.0 实测   说明
 *   breakableMin      33,174        33,495        ↑321（补词素解锁）
 *   truePrefixMin     75            63            ↓12：原 75 中含 12 词为假词根（уч-×11 + дн-×1），
 *                                                 v0.9.0 用真词根 суч- 等替换后这 12 词不再以「词首 gap」形态出现
 *                                                 ⇒ 现值 63 与 daCoreMin 精确重合（非巧合：落库后该类恰为 да- 族）
 *   daCoreMin         63            63            不变
 *   rNonEmptyMin      238           241           ↑3；R 率 30.09% → 30.47%
 *   l4Hole3Max        134           132           ↓2（改善）
 *   d1Count           0             0             不变（主断言）
 *
 *   ⇒ 三个地板常量（breakableMin / truePrefixMin / daCoreMin）本次**不上调**：
 *      上调会把「未来合法工作造成的下降」误报为红。当前值均满足 ⇒ 测试保持绿。
 *   ⇒ truePrefixMin 现值 63 是**零容差**，任何使该度量再降 1 的改动都会变红——
 *      这是有意的：v0.9.0 已把该度量的语义解释清楚（落库后它等价于「真前缀 + да- 族」），
 *      再降即意味着又有词根被误配回了 gap 形态。
 * ================================================================== */
export const FROZEN = {
  ruTotalMin: 101000, // 口径 A 分母地板（实测 101,512）
  ruPrefixMin: 185, // 俄语前缀词素数（实测 185，v0.9.0 未变：本次落库均为 root/suffix）
  singleCharPrefixes: ['в', 'о', 'с', 'у'], // 实测恰为这 4 个（v0.9.0 未变）
  d1Count: 0, // 全库 D1 模式计数（主断言）
  breakableMin: 33162, // 全库可拆俄语词条（v0.8.0 实测 33,174；v0.9.0 实测 33,495）
  truePrefixMin: 63, // 真前缀组地板（v0.8.0 实测 75；v0.9.0 实测 63 = 落库后含 да- 族 63）
  daCoreMin: 63, // 其中 `да-` 首片段子集（实测 63，0 容差；v0.9.0 未变）
  rSizeMin: 700, // 尺子 R 样本地板（实测 791）
  rNonEmptyMin: 238, // 尺子 R 可拆数（v0.9.0 实测 241 / 791 = 30.47%）
  rRateMin: 0.3, // 覆盖率率地板（拦单字级修过头：237/791 = 29.96% < 30.0%）
  l4Hole3Max: 134, // L4 空洞≥3（口径 A 总数），辅助护栏；v0.9.0 实测 132
};
